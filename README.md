...
100% of the original functionality and adds:
...
| 6 | Consistent device/entity naming | See below — device name no longer depends on which platform (`climate` vs `switch`) happens to load last |

Full details in [CHANGELOG.md](CHANGELOG.md).

### Device and entity naming

Each AC shows up as one device named **"Air Conditioner"**, with its Area
set from the location you enter during setup, and two entities beneath it:

- **`climate.<location>_air_conditioner`** — displays as just "Air Conditioner" (inherits the device name)
- **`switch.<location>_air_conditioner_led`** — displays as "Air Conditioner LED", controls the unit's front-panel status light

So in the UI you get a clean "Living Room → Air Conditioner", while
automations and scripts can reference the fully-qualified
`climate.living_room_air_conditioner` without ambiguity.

The location-based entity ID is generated automatically for **new** setups
from what you type into the "Location" field. It won't retroactively rename
entities you already have — see the migration note in
[CHANGELOG.md](CHANGELOG.md) if you're upgrading an existing install.
...
- Enter the **Location** (e.g. "Living Room") — this becomes the device's Area and the prefix for its entity IDs.
